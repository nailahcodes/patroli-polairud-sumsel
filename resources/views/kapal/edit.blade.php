...
@section('content')
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div>
            <h2 class="page-title mb-1">Edit Kapal</h2>
            <p class="text-muted mb-0">
                Perbarui data kapal patroli, zona, dan komandan kapal.
            </p>
        </div>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="kelompok" class="form-select" required>
                        <option value="Kelompok I" {{ old('kelompok', $kapal->kelompok) === 'Kelompok I' ? 'selected' : '' }}>Kelompok I</option>
                        <option value="Kelompok II" {{ old('kelompok', $kapal->kelompok) === 'Kelompok II' ? 'selected' : '' }}>Kelompok II</option>
                        <option value="Kelompok III" {{ old('kelompok', $kapal->kelompok) === 'Kelompok III' ? 'selected' : '' }}>Kelompok III</option>
                    </select>
                </div>

                <div class="col-md-4">
                    <label class="form-label">Kode Kapal</label>
                    <input type="text" name="kode_kapal" class="form-control" value="{{ old('kode_kapal', $kapal->kode_kapal) }}" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" class="form-select" required>
                        ...
                        <option value="nonaktif" {{ old('status', $kapal->status) === 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        <option value="perawatan" {{ old('status', $kapal->status) === 'perawatan' ? 'selected' : '' }}>Perawatan</option>
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="zona_patroli" id="zona_patroli" class="form-select" required>
                        ...
                        @foreach($zonaOptions as $zona => $wilayah)
                            <option value="{{ $zona }}" data-wilayah="{{ $wilayah }}" {{ old('zona_patroli', $kapal->zona_patroli) === $zona ? 'selected' : '' }}>
                                {{ $zona }} - {{ $wilayah }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="wilayah_patroli" id="wilayah_patroli" class="form-control" value="{{ old('wilayah_patroli', $kapal->wilayah_patroli) }}" readonly required>
                ...
                <div class="col-md-12">
                    ...
                    <select name="komandan_id" class="form-select" required>
                        ...
                        @foreach($komandans as $komandan)
                            <option value="{{ $komandan->id }}" {{ old('komandan_id', $kapal->komandan_id) == $komandan->id ? 'selected' : '' }}>
